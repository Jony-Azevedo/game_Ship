...
// A função deve ser chamada quando a tecla for pressionada
...
// A função deve ser chamada quando a tecla for liberada
function teclaUp(){
	var tecla=event.keyCode;
		...
		if(bombasTotal[i]){
			if(
				...
				(
					...
					((tiro.offsetLeft+6)>=(bombasTotal[i].offsetLeft)) // Parte direita do tiro com a parte esquerdad da bomba
				)
			){
...
function criaExplosao(tipo,x,y){ //Tipo 1=AR, 2=TERRA
	...
	if(tipo==1){
		...
		att2.value="top:"+y+"px;left:"+x+"px;";
		att4.value="assets/imagens/explosao_ar.gif?"+new Date();
	}else{
		att1.value="explosaoChao";
		att2.value="top:"+(tamTelaH-57)+"px;left:"+(x-17)+"px;";
		att4.value="assets/imagens/explosao_chao.gif?"+ new Date();
	}
	att5.value="assets/sons/exp1.mp3?"+new Date();
	att6.value="som"+isom;
...
function gerenciaGame(){
	...
	if(contBombas<=0){
		...
		telaMsg.style.backgroundImage='url("assets/imagens/vitoria.jpg")';
		telaMsg.style.display="block";
	}
	if(vidaPlaneta<=0){
		jogo=false;
		clearInterval(tmpCriaBomba);
		telaMsg.style.backgroundImage='url("assets/imagens/derrota.png")';
		telaMsg.style.display="block";
	}
...
function inicia(){
	...
	telaMsg.style.backgroundImage='url("assets/imagens/intro.png")';
	telaMsg.style.display="block";
...
// Faz a inicialização dos componentes, varáveis para o game ficar pronto para ser jogado
// Eventos de click de "keydow" e "keyup"
